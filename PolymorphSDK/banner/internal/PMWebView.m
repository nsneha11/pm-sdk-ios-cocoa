...
static BOOL const kPMAllowsInlineMediaPlaybackDefault = YES;
static BOOL const kPMRequiresUserActionForMediaPlaybackDefault = NO;

// Set defaults for this as its default differs between UIWebView and WKWebView
static BOOL const kPMAllowsLinkPreviewDefault = NO;

static NSString *const kPMJavaScriptDisableDialogScript = @"window.alert = function() { }; window.prompt = function() { }; window.confirm = function() { };";
static NSString *const kPMScalesPageToFitScript = @"var meta = document.createElement('meta'); meta.setAttribute('name', 'viewport'); meta.setAttribute('content', 'width=device-width, initial-scale=1.0, user-scalable=no'); document.getElementsByTagName('head')[0].appendChild(meta);";

static NSString *const kPMFrameKeyPathString = @"frame";

@interface PMWebView () <UIWebViewDelegate, WKNavigationDelegate, WKUIDelegate, UIScrollViewDelegate>
...
- (void)setUpStepsForceUIWebView:(BOOL)forceUIWebView {
    ...
    if (!forceUIWebView && [WKWebView class]) {
        ...
        config.allowsInlineMediaPlayback = kPMAllowsInlineMediaPlaybackDefault;
        if ([config respondsToSelector:@selector(requiresUserActionForMediaPlayback)]) {
            config.requiresUserActionForMediaPlayback = kPMRequiresUserActionForMediaPlaybackDefault;
        } else {
            config.mediaPlaybackRequiresUserAction = kPMRequiresUserActionForMediaPlaybackDefault;
        }
        ...
        self.uiWebView.allowsInlineMediaPlayback = kPMAllowsInlineMediaPlaybackDefault;
        self.uiWebView.mediaPlaybackRequiresUserAction = kPMRequiresUserActionForMediaPlaybackDefault;
    ...
    self.allowsLinkPreview = kPMAllowsLinkPreviewDefault;
    
    // set up KVO to adjust the frame of the WKWebView to avoid white screens
    if (self.wkWebView) {
        [self addObserver:self
               forKeyPath:kPMFrameKeyPathString
                  options:NSKeyValueObservingOptionOld
                  context:NULL];
    }
}

// WKWebView won't load/execute javascript unless it's on the view hierarchy. Because the Polymorph SDK uses a lot of
...
- (void)didMoveToWindow {
    // If using WKWebView, and if PMWebView is in the view hierarchy, and if the WKWebView is in the offscreen view currently,
    // move our WKWebView to self and deallocate OffscreenView if no other PMWebView is using it.
    if (self.wkWebView
        && self.window != nil
...
// Occasionally, we encounter an issue where, when PMWebView is initialized at a different frame size than when it's shown,
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if ([keyPath isEqualToString:kPMFrameKeyPathString]
        && [self.wkWebView.superview isEqual:gOffscreenView]) {
        self.wkWebView.frame = self.bounds;
    }
}
...
- (void)dealloc {
    // Remove KVO observer
    if (self.wkWebView) {
        [self removeObserver:self forKeyPath:kPMFrameKeyPathString];
    }
    ...
    if (self.uiWebView) {
        ...
        if (scalesPageToFit) {
            ...
            if (self.wkWebView.configuration.userContentController.userScripts.count == 0) {
                WKUserScript *viewportScript = [[WKUserScript alloc] initWithSource:kPMScalesPageToFitScript
...
shouldStartLoadWithRequest:(NSURLRequest *)request
 navigationType:(UIWebViewNavigationType)navigationType {
    if ([self.delegate respondsToSelector:@selector(pmWebView:shouldStartLoadWithRequest:navigationType:)]) {
        return [self.delegate pmWebView:self
...
- (void)webViewDidStartLoad:(UIWebView *)webView {
    if ([self.delegate respondsToSelector:@selector(pmWebViewDidStartLoad:)]) {
        [self.delegate pmWebViewDidStartLoad:self];
    }
}

- (void)webViewDidFinishLoad:(UIWebView *)webView {
    self.isViewLoaded = YES;
    if ([self.delegate respondsToSelector:@selector(pmWebViewDidFinishLoad:)]) {
        [self.delegate pmWebViewDidFinishLoad:self];
    }
}

- (void)webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error {
    if ([self.delegate respondsToSelector:@selector(pmWebView:didFailLoadWithError:)]) {
        [self.delegate pmWebView:self didFailLoadWithError:error];
    }
}

#pragma mark - UIWebView+PMAdditions methods
...
- (void)disableJavaScriptDialogs
{
    if (self.uiWebView) { // Only redefine on UIWebView, as the WKNavigationDelegate for WKWebView takes care of this.
        [self stringByEvaluatingJavaScriptFromString:kPMJavaScriptDisableDialogScript];
    }
}
...
decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    if ([self.delegate respondsToSelector:@selector(pmWebView:shouldStartLoadWithRequest:navigationType:)]) {
        ...
        policy = [self.delegate pmWebView:self
             shouldStartLoadWithRequest:request
