...
@interface PMTableViewAdPlacer () <UITableViewDataSource, UITableViewDelegate, StreamAdPlacerDelegate, PMNativeAdTrackerDelegate>
...
@implementation PMTableViewAdPlacer
...
{
    return [[self class] placerWithTableView:tableView viewController:controller adPositions:[PMServerAdPositions positioning] defaultAdRenderingClass:defaultAdRenderingClass];
}

+ (instancetype)placerWithTableView:(UITableView *)tableView viewController:(UIViewController *)controller adPositions:(PMAdPositions *)positions defaultAdRenderingClass:(Class)defaultAdRenderingClass
{
    PMTableViewAdPlacer *tableViewAdPlacer = [[PMTableViewAdPlacer alloc] initWithTableView:tableView viewController:controller adPositioning:positions defaultAdRenderingClass:defaultAdRenderingClass];
    return tableViewAdPlacer;
}

- (instancetype)initWithTableView:(UITableView *)tableView viewController:(UIViewController *)controller adPositioning:(PMAdPositions *)positioning defaultAdRenderingClass:(Class)defaultAdRenderingClass
{
...
- (void)loadAdsForAdUnitID:(NSString *)adUnitID targeting:(PMAdRequestTargeting *)targeting
{
    if (!self.insertionTimer) {
        self.insertionTimer = [IANTimer timerWithTimeInterval:kUpdateCellVisibilityInterval target:self selector:@selector(updateVisibleCells) repeats:YES];
...
{
    NSArray *visiblePaths = self.tableView.pm_indexPathsForVisibleRows;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.streamAdPlacer isAdAtIndexPath:indexPath]) {
        //This is to modify the default rendering class value after fetching sdk configs
        ...
        UITableViewCell<PMAdRendering> *cell = nil;
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= AN_IOS_6_0
            cell = (UITableViewCell<PMAdRendering> *)[tableView dequeueReusableCellWithIdentifier:identifier forIndexPath:indexPath];
#endif
        }
        
        if (!cell) {
            cell = (UITableViewCell<PMAdRendering> *)[tableView dequeueReusableCellWithIdentifier:identifier];
        }
...
{
    NSInvocation *invocation = [AdPlacerInvocation invokeForTarget:self.originalDataSource with2ArgSelector:@selector(tableView:canMoveRowAtIndexPath:) firstArg:tableView secondArg:indexPath streamAdPlacer:self.streamAdPlacer];
...
{
    [AdPlacerInvocation invokeForTarget:self.originalDataSource with3ArgIntSelector:@selector(tableView:commitEditingStyle:forRowAtIndexPath:) firstArg:tableView secondArg:editingStyle thirdArg:indexPath streamAdPlacer:self.streamAdPlacer];
}
...
{
    [AdPlacerInvocation invokeForTarget:self.originalDelegate with3ArgSelector:@selector(tableView:willDisplayCell:forRowAtIndexPath:) firstArg:tableView secondArg:cell thirdArg:indexPath streamAdPlacer:self.streamAdPlacer];
}

- (void)tableView:(UITableView *)tableView didEndDisplayingCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    [AdPlacerInvocation invokeForTarget:self.originalDelegate with3ArgSelector:@selector(tableView:didEndDisplayingCell:forRowAtIndexPath:) firstArg:tableView secondArg:cell thirdArg:indexPath streamAdPlacer:self.streamAdPlacer];
}

- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath
{
    [AdPlacerInvocation invokeForTarget:self.originalDelegate with2ArgSelector:@selector(tableView:accessoryButtonTappedForRowWithIndexPath:) firstArg:tableView secondArg:indexPath streamAdPlacer:self.streamAdPlacer];
}

- (BOOL)tableView:(UITableView *)tableView shouldHighlightRowAtIndexPath:(NSIndexPath *)indexPath
{
    NSInvocation *invocation = [AdPlacerInvocation invokeForTarget:self.originalDelegate with2ArgSelector:@selector(tableView:shouldHighlightRowAtIndexPath:) firstArg:tableView secondArg:indexPath streamAdPlacer:self.streamAdPlacer];
...
{
    [AdPlacerInvocation invokeForTarget:self.originalDelegate with2ArgSelector:@selector(tableView:didHighlightRowAtIndexPath:) firstArg:tableView secondArg:indexPath streamAdPlacer:self.streamAdPlacer];
}

- (void)tableView:(UITableView *)tableView didUnhighlightRowAtIndexPath:(NSIndexPath *)indexPath
{
    [AdPlacerInvocation invokeForTarget:self.originalDelegate with2ArgSelector:@selector(tableView:didUnhighlightRowAtIndexPath:) firstArg:tableView secondArg:indexPath streamAdPlacer:self.streamAdPlacer];
}
...
{
    [AdPlacerInvocation invokeForTarget:self.originalDelegate with2ArgSelector:@selector(tableView:didDeselectRowAtIndexPath:) firstArg:tableView secondArg:indexPath streamAdPlacer:self.streamAdPlacer];
}
...
{
    NSInvocation *invocation = [AdPlacerInvocation invokeForTarget:self.originalDelegate with2ArgSelector:@selector(tableView:titleForDeleteConfirmationButtonForRowAtIndexPath:) firstArg:tableView secondArg:indexPath streamAdPlacer:self.streamAdPlacer];
    
    return [AdPlacerInvocation resultForInvocation:invocation defaultValue:@"Delete"];
}

- (BOOL)tableView:(UITableView *)tableView shouldIndentWhileEditingRowAtIndexPath:(NSIndexPath *)indexPath
{
    NSInvocation *invocation = [AdPlacerInvocation invokeForTarget:self.originalDelegate with2ArgSelector:@selector(tableView:shouldIndentWhileEditingRowAtIndexPath:) firstArg:tableView secondArg:indexPath streamAdPlacer:self.streamAdPlacer];
...
{
    [AdPlacerInvocation invokeForTarget:self.originalDelegate with2ArgSelector:@selector(tableView:willBeginEditingRowAtIndexPath:) firstArg:tableView secondArg:indexPath streamAdPlacer:self.streamAdPlacer];
}

- (void)tableView:(UITableView *)tableView didEndEditingRowAtIndexPath:(NSIndexPath *)indexPath
{
    [AdPlacerInvocation invokeForTarget:self.originalDelegate with2ArgSelector:@selector(tableView:didEndEditingRowAtIndexPath:) firstArg:tableView secondArg:indexPath streamAdPlacer:self.streamAdPlacer];
}

- (NSInteger)tableView:(UITableView *)tableView indentationLevelForRowAtIndexPath:(NSIndexPath *)indexPath
{
    NSInvocation *invocation = [AdPlacerInvocation invokeForTarget:self.originalDelegate with2ArgSelector:@selector(tableView:indentationLevelForRowAtIndexPath:) firstArg:tableView secondArg:indexPath streamAdPlacer:self.streamAdPlacer];
    
    return [AdPlacerInvocation integerResultForInvocation:invocation
                                               defaultValue:UITableViewCellEditingStyleNone];
}

- (BOOL)tableView:(UITableView *)tableView shouldShowMenuForRowAtIndexPath:(NSIndexPath *)indexPath
{
    NSInvocation *invocation = [AdPlacerInvocation invokeForTarget:self.originalDelegate with2ArgSelector:@selector(tableView:shouldShowMenuForRowAtIndexPath:) firstArg:tableView secondArg:indexPath streamAdPlacer:self.streamAdPlacer];
...
@implementation UITableView (PMTableViewAdPlacer)

static char kAdPlacerKey;

- (void)pm_setAdPlacer:(PMTableViewAdPlacer *)placer
{
    objc_setAssociatedObject(self, &kAdPlacerKey, placer, OBJC_ASSOCIATION_ASSIGN);
}

- (PMTableViewAdPlacer *)pm_adPlacer
{
    return objc_getAssociatedObject(self, &kAdPlacerKey);
}

- (void)pm_setDelegate:(id<UITableViewDelegate>)delegate
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
...
- (id<UITableViewDelegate>)pm_delegate
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
...
- (void)pm_setDataSource:(id<UITableViewDataSource>)dataSource
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
...
- (id<UITableViewDataSource>)pm_dataSource
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
...
- (CGRect)pm_rectForRowAtIndexPath:(NSIndexPath *)indexPath
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
...
- (NSIndexPath *)pm_indexPathForRowAtPoint:(CGPoint)point
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
...
- (NSIndexPath *)pm_indexPathForCell:(UITableViewCell *)cell
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
...
- (NSArray <NSIndexPath *> *)pm_indexPathsForRowsInRect:(CGRect)rect
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
...
- (UITableViewCell *)pm_cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
    NSIndexPath *adjustedIndexPath = indexPath;
...
- (NSArray *)pm_visibleCells
{
    ...
    if (adPlacer) {
        NSArray *indexPaths = [self pm_indexPathsForVisibleRows];
        NSMutableArray *visibleCells = [NSMutableArray array];
        for (NSIndexPath *indexPath in indexPaths) {
            [visibleCells addObject:[self pm_cellForRowAtIndexPath:indexPath]];
        }
        return visibleCells;
    } else {
...
- (NSArray <NSIndexPath *> *)pm_indexPathsForVisibleRows
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
...
- (void)pm_scrollToRowAtIndexPath:(NSIndexPath *)indexPath atScrollPosition:(UITableViewScrollPosition)scrollPosition animated:(BOOL)animated
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
    NSIndexPath *adjustedIndexPath = indexPath;
...
- (void)pm_insertSections:(NSIndexSet *)sections withRowAnimation:(UITableViewRowAnimation)animation
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
...
- (void)pm_deleteSections:(NSIndexSet *)sections withRowAnimation:(UITableViewRowAnimation)animation
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
...
- (void)pm_reloadSections:(NSIndexSet *)sections withRowAnimation:(UITableViewRowAnimation)animation
{
...
- (void)pm_insertRowsAtIndexPaths:(NSArray *)indexPaths withRowAnimation:(UITableViewRowAnimation)animation
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
...
- (void)pm_deleteRowsAtIndexPaths:(NSArray *)indexPaths withRowAnimation:(UITableViewRowAnimation)animation
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
    NSArray *adjustedIndexPaths = indexPaths;
    
    // We need to wrap the delete process in begin/end updates in case any ad
    // cells are also deleted. StreamAdPlacer's deleteItemsAtIndexPaths: can
    // call the delegate's didRemoveAdsAtIndexPaths, which will remove those
    // ads from the tableview.
    [self pm_beginUpdates];
    if (adPlacer) {
...
- (void)pm_reloadRowsAtIndexPaths:(NSArray *)indexPaths withRowAnimation:(UITableViewRowAnimation)animation
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
    NSArray *adjustedIndexPaths = indexPaths;
...
- (void)pm_moveRowAtIndexPath:(NSIndexPath *)indexPath toIndexPath:(NSIndexPath *)newIndexPath
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
...
- (NSIndexPath *)pm_indexPathForSelectedRow
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
...
- (NSArray <NSIndexPath *> *)pm_indexPathsForSelectedRows
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
...
- (void)pm_selectRowAtIndexPath:(NSIndexPath *)indexPath animated:(BOOL)animated scrollPosition:(UITableViewScrollPosition)scrollPosition
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
    NSIndexPath *adjustedIndexPath = indexPath;
...
- (void)pm_deselectRowAtIndexPath:(NSIndexPath *)indexPath animated:(BOOL)animated
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
    NSIndexPath *adjustedIndexPath = indexPath;
...
- (id)pm_dequeueReusableCellWithIdentifier:(NSString *)identifier forIndexPath:(NSIndexPath *)indexPath
{
    PMTableViewAdPlacer *adPlacer = [self pm_adPlacer];
    NSIndexPath *adjustedIndexPath = indexPath;
