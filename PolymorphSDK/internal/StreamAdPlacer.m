...
#import "PMServerAdPositions.h"
#import "SDKConfigs.h"
#import "SDKConfigsSource.h"
#import "Constants.h"

static NSString * const kReuseIdentifierPrefix = @"AdsNative";
static NSInteger const kAdInsertionLookAheadAmount = 3;
static const NSUInteger kIndexPathItemIndex = 1;

@interface StreamAdPlacer () <AdSourceDelegate, AdDelegate>

@property (nonatomic, strong) AdSource *adSource;
@property (nonatomic, strong) SDKConfigsSource *configSource;
@property (nonatomic, copy) PMAdPositions *adPositioning;
@property (nonatomic, strong) StreamAdPlacementData *adPlacementData;
...
@implementation StreamAdPlacer

+ (instancetype)placerWithViewController:(UIViewController *)controller adPositions:(PMAdPositions *)positions defaultAdRenderingClass:(Class)defaultAdRenderingClass
{
    StreamAdPlacer *placer = [[self alloc] initWithViewController:controller adPositioning:positions defaultAdRenderingClass:defaultAdRenderingClass];
    return placer;
}

- (instancetype)initWithViewController:(UIViewController *)controller adPositioning:(PMAdPositions *)positioning defaultAdRenderingClass:(Class)defaultAdRenderingClass
{
    ...
    NSAssert([defaultAdRenderingClass conformsToProtocol:@protocol(PMAdRendering)] && [defaultAdRenderingClass isSubclassOfClass:[UIView class]], @"A stream ad placer must be instantiated with a class that implements AdRendering and is a view.");
    
    self = [super init];
    if (self) {
        _viewController = controller;
        _adPositioning = [positioning copy];
        _adSource = [[InstanceProvider sharedProvider] buildNativeAdSourceWithDelegate:self];
        _adPlacementData = [[InstanceProvider sharedProvider] buildStreamAdPlacementDataWithPositioning:_adPositioning];
...
{
    NativeAdData *adData = [self.adPlacementData adDataAtAdjustedIndexPath:indexPath];
...
- (void)renderAdAtIndexPath:(NSIndexPath *)indexPath inView:(UIView<PMAdRendering> *)view
{
    NativeAdData *adData = [self.adPlacementData adDataAtAdjustedIndexPath:indexPath];
    
    if (!adData) {
        LogError(@"-renderAdAtIndexPath: An ad does not exist at indexPath");
        return;
    }
...
- (void)loadAdsForAdUnitID:(NSString *)adUnitID targeting:(PMAdRequestTargeting *)targeting
{
    ...
    //Load configs from the AdsNative server
    if(adUnitID) {
        
        // Reset to a placement data that has no "desired" ads at all.
        self.adPlacementData = [[InstanceProvider sharedProvider] buildStreamAdPlacementDataWithPositioning:nil];
        
        self.configSource = [SDKConfigsSource sharedInstance];
        __typeof__(self) __weak weakSelf = self;
        [self.configSource loadConfigsWithAdUnitIdentifier:self.adUnitID completionHandler:^(SDKConfigs *configs, NSError *error) {
            ...
            if (error) {
                if ([error code] == SDKConfigsSourceEmptyResponse && !([self.adPositioning isKindOfClass:[PMClientAdPositions class]])) {
                    LogError(@"ERROR: Ad placer cannot show any ads because ad positions have "
                             @"not been configured for your ad unit %@. You must assign positions "
                             @"by editing the ad unit's settings on the AdsNative website.",
                             strongSelf.adUnitID);
                } else {
                    LogError(@"ERROR: Ad placer failed to get positions from the ad server for "
                             @"ad unit ID %@. Error: %@", strongSelf.adUnitID, error);
                }
            } else {
                if (configs.renderingClass != nil) {
                    strongSelf.defaultAdRenderingClass = configs.renderingClass;
                }
                
                
                if (([_adPositioning isKindOfClass:[PMClientAdPositions class]])) {
                    strongSelf.adPlacementData = [[InstanceProvider sharedProvider] buildStreamAdPlacementDataWithPositioning:_adPositioning];
                } else {
                    strongSelf.adPlacementData = [[InstanceProvider sharedProvider] buildStreamAdPlacementDataWithPositioning:configs.positions];
                }
    ...
    if (!adUnitID) {
        ...
        self.adPlacementData = [[InstanceProvider sharedProvider] buildStreamAdPlacementDataWithPositioning:nil];
    } else if ([self.adPositioning isKindOfClass:[PMClientAdPositions class]]) {
        // Reset to a placement data that has "desired" ads but not "placed" ones.
        self.adPlacementData = [[InstanceProvider sharedProvider] buildStreamAdPlacementDataWithPositioning:self.adPositioning];
    } else if ([self.adPositioning isKindOfClass:[PMServerAdPositions class]]) {
        //do nothing - handled in sdk config request from server
    ...
    if (!adUnitID) {
        LogError(@"Ad placer cannot load ads with a nil ad unit ID.");
        return;
    }
    
    [self.adSource loadAdsWithAdUnitIdentifier:adUnitID andTargeting:targeting withViewController:self.viewController];
}
...
- (NativeAdData *)retrieveAdDataForInsertionPath:(NSIndexPath *)insertionPath
{
    PMNativeAd *adObject = [self.adSource dequeueAdForAdUnitIdentifier:self.adUnitID];
    
    if (!adObject) {
        return nil;
    }
    
    NativeAdData *adData = [[NativeAdData alloc] init];
...
- (void)fillAdsInConsideredRange
{
    ...
    while ([self shouldPlaceAdAtIndexPath:insertionPath]) {
        NativeAdData *adData = [self retrieveAdDataForInsertionPath:insertionPath];
        adData.ad.internalDelegate = self;
        ...
        if (adData.ad.isBackupClassRequired) {
            NSString *backupClass = [backupClassPrefix stringByAppendingString:
                                     NSStringFromClass(self.defaultAdRenderingClass)];
            ...
            if ([backupRenderingClass conformsToProtocol:@protocol(PMAdRendering)] && [backupRenderingClass isSubclassOfClass:[UIView class]]) {
                self.defaultAdRenderingClass = backupRenderingClass;
            } else {
...
{
    if ([self.trackerDelegate respondsToSelector:@selector(anNativeAdDidRecordImpression)]) {
        [self.trackerDelegate anNativeAdDidRecordImpression];
    }
}

- (BOOL)anNativeAdDidClick:(PMNativeAd *)nativeAd
{
    if ([self.trackerDelegate respondsToSelector:@selector(anNativeAdDidClick:)]) {
        return [self.trackerDelegate anNativeAdDidClick:nativeAd];
    }
    return NO;
}
