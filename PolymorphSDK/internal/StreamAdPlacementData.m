...
@interface StreamAdPlacementData ()
...
@implementation StreamAdPlacementData

- (id)initWithPositioning:(PMAdPositions *)positioning
{
    self = [super init];
    if (self) {
...
- (void)initializeDesiredPositionsFromPositioning:(PMAdPositions *)positioning
{
...
- (void)insertAdData:(NativeAdData *)data atIndexPath:(NSIndexPath *)adjustedIndexPath
{
    ...
    if (indexInDesiredArrays == NSNotFound) {
        LogWarn(@"Attempted to insert an ad at position %@, which is not in the desired array.", adjustedIndexPath);
        return;
    }
