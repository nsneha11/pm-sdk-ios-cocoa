...
@protocol StreamAdPlacerDelegate;
@protocol PMAdRendering;
@class PMAdRequestTargeting;
@protocol PMNativeAdTrackerDelegate;

/**
 * The `StreamAdPlacer` class allows you to retrieve native ads from the AdsNative ad server and
 ...
 * `StreamAdPlacerDelegate` protocol to handle ad insertions.
 *
...
@interface StreamAdPlacer : NSObject
...
@property (nonatomic, weak) id<StreamAdPlacerDelegate> delegate;
@property (nonatomic, weak) id<PMNativeAdTrackerDelegate> trackerDelegate;
@property (nonatomic, readonly, copy) PMAdPositions *adPositioning;
 ...
 * of `UIView` and also needs to implement the `PMAdRendering` protocol.
 */
...
@protocol StreamAdPlacerDelegate <NSObject>
