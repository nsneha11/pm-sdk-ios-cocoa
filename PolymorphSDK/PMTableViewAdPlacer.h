...
#import "PMTableViewAdPlacerDelegate.h"
...
/**
 * The `PMTableViewAdPlacer` class allows you to request native ads from the AdsNative ad server and
 ...
 * content cells.
 */
@interface PMTableViewAdPlacer : NSObject
 ...
 * `PMServerAdPositions` object as the `positions` parameter.
 *
 ...
 * implement the `PMAdRendering` protocol and must be a subclass of `UITableViewCell`.
 *
 * @return An `PMTableViewAdPlacer` object.
 */
 ...
 *
 * First, you may pass an `PMServerAdPositions` object as the `positions` parameter, which tells
 ...
 * this as well.
 *
 * Alternatively, if you wish to hard-code your positions, you may pass an `PMClientAdPositions`
 * object instead.
 *
 * @param tableView The table view in which to insert ads.
 * @param controller The view controller which should be used to present modal content.
...
/**
 * The delegate of an `PMTableViewAdPlacer` instance may choose to
 * implement the `PMTableViewAdPlacerDelegate` protocol
 * to get callbacks for impressions and clicks
 */
@property (nonatomic, weak) id<PMTableViewAdPlacerDelegate> delegate;
 ...
 * responsibilities related to `PMTableViewAdPlacer`. These APIs include methods to help notify the
...
@interface UITableView (PMTableViewAdPlacer)
